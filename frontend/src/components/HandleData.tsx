...
export default function HandleData({data} : HandleDataProps){
    ...
    return(
        <div className="space-y-6 text-white">
            ...
            <p className="text-log text-center font-bold">{data.description}</p>
            ...
            <div className="mt-20 flex flex-col gap-6">
                {links.length ? 
                    links.map(link =>(
                        <a
                            key={link.name}
                            className="bg-white px-5 py-3 flex items-center gab-5 rounded-lg"
                            href={link.url}
                            target="_blank"
                            rel="noreferrer noopener"

                        >
                            <img src={`/social/icon_${link.name}.svg`} alt="imagen red social" className="w-12"></img>
